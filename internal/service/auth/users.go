...
func (u *UsersModel) Login(userName string, pass string) *UsersModel {
	...
	if result.Error == nil {
		...
		logger.Log.Error("根据账号查询单条记录出错:", result.Error)
	}
	return nil
}
...
func (u *UsersModel) counts(userName string) (counts int64) {
	...
	if res := global.DB.Raw(sql, "%"+userName+"%").First(&counts); res.Error != nil {
		logger.Log.Error("UsersModel - counts 查询数据条数出错", res.Error)
	}
	return counts
}
...
//		logger.Log.Error("redis连接失败，请检查配置")
//		return
//	}
...
//					}
//				} else {
//					logger.Log.Error("expires_at 转换位时间戳出错", zap.Error(err))
//				}
//			}
//		}
//	}
